...
from stable_baselines3_thesis.common.preprocessing import get_flattened_obs_dim, is_image_space
from stable_baselines3_thesis.common.type_aliases import TensorDict
from stable_baselines3_thesis.common.utils import get_device
...
class MlpExtractor(nn.Module):
    ...
    def __init__(
        ...
        # For Captum's forward function
        self.gradcam_forward = False
    ...
    def forward(self, features: th.Tensor, ret_act=False) -> Tuple[th.Tensor, th.Tensor]:
        """
        ...
        if ret_act == True:
            ...
            value_feats = self.value_net(shared_latent)

            policy_feats = shared_latent
            policy_act_list = []
            for layer in self.policy_net.children():
                policy_feats = layer(policy_feats)
                if isinstance(layer,nn.ReLU) or isinstance(layer,nn.Tanh):
                    policy_act_list.append(policy_feats)
                    
            policy_act_1D = th.hstack(policy_act_list)

            return policy_feats, value_feats, policy_act_1D
        elif ret_act == False and self.gradcam_forward == False:
            shared_latent = self.shared_net(features)
